...
MeterIPAddress = '10.173.200.1'  # IP address
...
# Modbus client
client = ModbusTcpClient(host=MeterIPAddress, port=205, timeout=20)
...
class ModbusNetworkConfiguration:
    ...
    def GetHMINetworkSettings(self):
        try:
            ...
            subnet_match = re.search(subnet_pattern, output) if self.system == "Windows" else None

            if ip_match:
                self.HMIIPAddress = ip_match.group(1)
                if self.system == "Windows" and subnet_match:
                    self.HMISubnet = subnet_match.group(1)
                elif self.system == "Darwin":
                    subnet_mask_hex = ip_match.group(2)
                    self.HMISubnet = '.'.join([str(int(subnet_mask_hex[i:i + 2], 16)) for i in range(2, 10, 2)])
                elif self.system == "Linux":
                    cidr = int(ip_match.group(2))
                    self.HMISubnet = '.'.join(
                        [str((0xffffffff << (32 - cidr) >> i) & 0xff) for i in [24, 16, 8, 0]])
                else:
    ...
    def ReadData(self):
        ...
        if client.connect():
            try:
                ...
                # Get CT and PT Ratios
                self.CTprimaryCurrent = client.read_holding_registers(address=2306, count=1,
                                                                      slave=MeterNodeAddress).getRegister(0)
                PTratio = client.read_holding_registers(address=2305, count=1, slave=MeterNodeAddress).getRegister(
                    0) * 0.1
                ...
                DeviceResolution = client.read_holding_registers(address=2390, count=1,
                                                                 slave=MeterNodeAddress).getRegister(0)
                ...
                raw_data1 = client.read_holding_registers(address=13952, count=65, slave=MeterNodeAddress)
                ...
                raw_data2 = client.read_holding_registers(address=14336, count=26, slave=MeterNodeAddress)
                ...
                raw_data3 = client.read_holding_registers(address=14466, count=4, slave=MeterNodeAddress)
                ...
                raw_data4 = client.read_holding_registers(address=7324, count=7, slave=MeterNodeAddress)
...
class DataLogger:
    ...
    def ReadDatalogger(self, DataLogNo):
        ...
        if client.connect():
            self.TwoDArray = []
            client.write_registers(address=44378, values=[9, 0], slave=MeterNodeAddress)

            response = client.read_holding_registers(44378, 1, 2)
            value = response.registers[0]

            # Interpret the value correctly
            if value == 65535:
                print("Authorization required (-1)")
            elif value == 0:
                print("Access permitted (0)")
            else:
            ...
            # Request file info
            client.write_register(address=64944, value=9, slave=MeterNodeAddress)
            client.write_register(address=64945, value=self.LogFileID, slave=MeterNodeAddress)
            ...
            FileResponseBlock = client.read_holding_registers(address=63152, count=8, slave=MeterNodeAddress)
            ...
            client.write_register(address=63121, value=self.LogFileID, slave=MeterNodeAddress)
            ...
            if self.start_time and self.end_time:
                ...
                if response.isError():
                    print('getting end time to now error')
                ...
                client.write_registers(address=63128, values=start_bits,
                                       slave=MeterNodeAddress)  # Low part of start time
                ...
                response = client.read_holding_registers(address=63160, count=1, slave=MeterNodeAddress)
                if response.isError():
                    print('getting start time to now error')
                    print(f"Error reading response: {response}")
                    return None
                else:
                    FileInfoBlock = client.read_holding_registers(address=64960, count=36, slave=MeterNodeAddress)
            ...
            while ReadRecordNo <= TimeRangeRecordNo:
                ...
                for i in range(0, BlockRecordNo):
                    # print(ReadRecordNo)
                    # Read current record
                    ReadRegSet = client.read_holding_registers(address=DataBufferRegister, count=RecordSize,
                                                               slave=MeterNodeAddress)
                    ...
                    # check for valid data before processing, aviod empty or padded records
                    if TimeCal == 0:
                        continue
...
class EventLogger:
    ...
    def ReadEventlogger(self):
        ...
        if client.connect():
            ...
            # Clear Buffer
            client.write_register(address=63120, value=1, slave=MeterNodeAddress)
            client.write_register(address=63121, value=self.LogFileID, slave=MeterNodeAddress)  # Not Working

            # Request file info
            client.write_register(address=64944, value=9, slave=MeterNodeAddress)
            rq = client.write_register(address=64945, value=self.LogFileID, slave=MeterNodeAddress)
            if rq.isError():
                print("rq error")
                return 0
