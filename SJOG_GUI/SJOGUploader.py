...
def process_xlsx_file(file_path, table_dict, cursor, site):
    try:
        from Waste_xlsx_Format import waste
        from veolia_formatter import process_veolia_xlsx
        from pandas import ExcelFile
        with ExcelFile(file_path) as excel:
            sheet_count = len(excel.sheet_names)

            if sheet_count == 1:
                df = process_veolia_xlsx(file_path, site=site)
                upload_dataframe_to_azure_sql(df, "WasteVeolia", cursor, table_dict)
            else:
                df = waste(file_path)
                df["Site"] = site
                upload_dataframe_to_azure_sql(df[:-1], "TestingWWaste", cursor, table_dict)
...
def process_pdf_file(file_path, table_dict, cursor, site):
    try:
        ...
        from Water_Bill import water, bill
...
def process_files_in_directory(directory, cursor, table_dict):
    for root, _, files in os.walk(directory):
        ...
        for filename in files:
            ...
            print(f"Processing file: {filename} in site: {site}")
            if extension in ['.xlsx', '.xls', 'xlsm']:
                process_xlsx_file(file_path, table_dict, cursor, site)
            elif extension == '.csv':
                process_csv_file(file_path, table_dict, cursor, site)
            elif extension == '.pdf':
                process_pdf_file(file_path, table_dict, cursor, site)
...
def upload_tem(cursor, table_dict):
